...
public:
	FileStream(const char* lpFileName, unsigned int dwAccessType);
	FileStream(const char* lpFileName, unsigned int dwAccessType, OpenDisposition eWin32CreateionDisposition);
	~FileStream();
...
public:
	...
	//从字节流加载数据到内存流
	int copyFromChar(char* buffer, const unsigned long size);
};
...
public:
	ZDecompressionStream(BaseStream& source);
