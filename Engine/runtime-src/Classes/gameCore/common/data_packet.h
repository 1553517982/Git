...
/***********************************************************************
  ...
  数据包的内存申请时通过内存池对象BufferAllocator进行的，以使用更有效率的内存管理。
...
public:
	...
	{
		size_t mem_size = end_ptr_ - mem_ptr_;
		offset_ = data_end_ = mem_ptr_;
		memset(mem_ptr_, 0, mem_size);
	}
...
public:
	/* 写入原子数据的运算符重载 */
	template <typename T>
	inline DataPacket& operator << (T val)
	{
...
#ifdef _DPPACK_MEM_CHK_
			new_mem = (char*)allocator_->AllocBuffer(newSize + sizeof(int));
#else
			new_mem = (char*)allocator_->AllocBuffer(newSize);
#endif
		}
		else
		{
			new_mem = (char*)malloc(newSize);
		}
