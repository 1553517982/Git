#ifndef _DATA_PACKET_READER_H_
#define _DATA_PACKET_READER_H_
...
public:
	...
	inline T readAtom()
	{
		T val;
		size_t avaliable = data_end_ - offset_;
// Modified by Shan Lu 2013/04/03 Begin
//Problems:
...
//　　analyse:NDK's compiler may be different from linux or the cygwin is not perfect.
		...
		{
			memcpy(&val, offset_, readsize);
			offset_ += readsize;
		}
// Modified by Shan Lu 2013/04/03 End
		else if (avaliable > 0)
		{
	...
	* 字符串的数据格式为：[2字节字符长度数据][字符串字节数据，字符串长度在65536以内][字符串终止字符0]
	...
	{
		return rawReadStringLen(str, read_len);
	}
	/* 读取UNICODE16短字符串数据
	* 字符串的数据格式为：[2字节字符长度数据][字符串字节数据，字符串长度在65536以内][字符串终止字符0]
	 * str			 字符串读取缓冲区指针
...
protected:
	...
	size_t rawReadStringLen(char* str, size_t read_len)
	{
		...
		if (avaliable >= 2)
		{
			unsigned short* temp = (unsigned short*)offset_;
			rlen = str_len = *temp;
