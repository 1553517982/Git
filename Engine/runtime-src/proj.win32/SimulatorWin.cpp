...
SimulatorWin::~SimulatorWin()
{
//COCOS2D_CUSTOM_FIX Add Begin 8/4/2015 17:02 
//@brief: 
#ifdef COCOS2D_CUSTOM_FIX
	player::PlayerProtocol::purgeInstance();
	ConfigParser::purge();
	delete player::DeviceEx::getInstance();
	delete AppLang::getInstance();
	delete SimulatorConfig::getInstance();
#endif
//COCOS2D_CUSTOM_FIX Add End
...
int SimulatorWin::run()
{
...
#ifdef COCOS2D_CUSTOM_FIX
	FileUtils::getInstance()->createDirectory(_project.getWritableRealPath().c_str());
	...
	//fix FileUtils searchPath in debug mode before calling UserDefault
...
#ifdef COCOS2D_CUSTOM_FIX
//	CCAppConfig::setXMLFile(_project.getAppConfigPath().c_str());
...
LRESULT CALLBACK SimulatorWin::windowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		if (wParam == VK_F5)
		{
//			//CC_SAFE_DELETE(SimulatorWin::getInstance()->_app);
			_instance->relaunch();
//			CC_SAFE_DELETE(SimulatorWin::getInstance()->_app);
//			FreeConsole();
//			STARTUPINFO sinfo;
...
//#ifdef _DEBUG
//			::CreateProcess(L"cocosLuaEngine_debug.exe", L"", NULL, NULL, true, 0, NULL, NULL, &sinfo, &pinfo);
//#else
//			::CreateProcess(L"cocosLuaEngine.exe", L"", NULL, NULL, true, 0, NULL, NULL, &sinfo, &pinfo);
//#endif
//			::ExitProcess(0);
        }
        break;
